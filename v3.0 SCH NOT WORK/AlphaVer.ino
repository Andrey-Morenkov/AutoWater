...
void setup()
{
	...
	//sensor[0].setPin(hygrometers[0]);                // Гигрометр #1
	//sensor[1].setPin(hygrometers[1]);                // Гигрометр #2
	sensor[WS].setPin(waterSensor);                // Сенсор воды


	flower = new Plant[maxFlowers];
	//flower[0].setParams(valves[0], sensor[0]);        // Цветок #1          
	//flower[1].setParams(valves[1], sensor[1]);        // Цветок #2
